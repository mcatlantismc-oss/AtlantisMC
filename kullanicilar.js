(() => {
  ...
  function render() {
    const q = String(document.getElementById('users-search')?.value || '')
    ...
    const rows = all.filter(p =>
      String(p.username || '').toLocaleLowerCase('tr-TR').includes(q)
    );
    ...
    box.innerHTML = rows.map(p => {
      ...
      return `
        <button
          type="button"
          class="community-user-card"
          data-user-id="${esc(p.id)}"
          data-username="${esc(name)}"
          aria-label="${esc(name)} profilini görüntüle"
        >
          <span class="community-avatar">${avatar}</span>
          <span class="community-user-copy">
            <strong>${esc(name)}</strong>
            <span class="role-chip role-${esc(role)}">${esc(roleText(role))}</span>
            <small>${p.bio ? esc(p.bio) : 'Profilini görüntüle'}</small>
          </span>
        </button>
      `;
    }).join('');
  ...
  async function load() {
    ...
    try {
      ...
      box.innerHTML = `<div class="empty-panel">Kullanıcılar alınamadı: ${esc(e.message)}</div>`;
    }
  }
