-- Atlantis MC — FINAL moderation/chat control patch
-- Safe/idempotent additions. Does not delete users/messages.
begin;
...
begin
  ...
  if duration_seconds < 0 then raise exception 'invalid duration'; end if;

  select site_role into target_role from public.profiles where id = target_user_id;
  if target_role = 'admin' and own_role <> 'admin' then raise exception 'cannot ban admin'; end if;
...
-- Helpers for existing moderation role checks.
...
begin
  select site_role into own_role from public.profiles where id = auth.uid();
  if own_role not in ('admin','moderator') then raise exception 'no permission'; end if;
  if target_user_id = auth.uid() then raise exception 'cannot mute yourself'; end if;
  if duration_seconds < 0 then raise exception 'invalid duration'; end if;
  select site_role into target_role from public.profiles where id = target_user_id;
  if target_role = 'admin' and own_role <> 'admin' then raise exception 'cannot mute admin'; end if;
  if duration_seconds = 0 then until_time := null;
  else until_time := now() + make_interval(secs => duration_seconds);
  end if;
...
create function public.admin_set_role(target_user_id uuid, new_role text)
returns boolean
language plpgsql
security definer
set search_path = public
...
-- Replace chat RPCs so mute/ban/chat-lock rules are enforced server-side.
...
begin
  ...
  select username, muted_until, banned_until, avatar_url, site_role into p
  ...
  select created_at into last_time from public.messages where user_id = auth.uid() order by created_at desc limit 1;
  if last_time is not null and last_time > now() - interval '3 seconds' then raise exception 'Yeni mesaj göndermek için biraz bekle.'; end if;

  insert into public.messages(user_id, username, message, avatar_url)
  values(auth.uid(), p.username, clean, p.avatar_url)
  returning * into sent;

  return jsonb_build_object('ok',true,'id',sent.id,'user_id',sent.user_id,'username',sent.username,'message',sent.message,'created_at',sent.created_at,'edited_at',sent.edited_at,'avatar_url',sent.avatar_url);
end;
$$;
...
set search_path = public
as $$
declare clean text := btrim(p_message); sent public.messages;
begin
  ...
  returning * into sent;
  return jsonb_build_object('ok',true,'id',sent.id,'user_id',null,'username','Anonim','message',sent.message,'created_at',sent.created_at,'edited_at',sent.edited_at,'avatar_url',null);
end;
$$;
