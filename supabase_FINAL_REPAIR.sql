-- Atlantis MC — FINAL one-time Supabase repair
-- Keeps existing users and messages.
...
begin
  ...
  if not found then
    ...
    values(
      auth.uid(), null, new_avatar_url, coalesce(new_bio,''), now()
    )
...
-- Profile images: Supabase Storage public bucket, uploads still owner-controlled.
