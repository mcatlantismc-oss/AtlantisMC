...
(function(){
  ...
  window.openAtlantisProfileCard =
    showAtlantisProfileCard;

  window.openAtlantisProfileEditor =
    openAtlantisProfileEditor;

  window.getAtlantisCurrentProfile =
  ...
  function usernameStorageKey(userId){
    return 'atlantis.username.' + String(userId || '');
  }
  ...
  function rememberUsername(userId, username){
    ...
    try{
      localStorage.setItem(
        usernameStorageKey(userId),
        String(username).trim()
      );
    }catch{}
  }

  function rememberedUsername(userId){
    if(!userId) return '';
    try{
      return String(
  ...
  async function recoverProfile(user){
    if(!user) return null;
    ...
    for(const candidate of candidates){
      try{
        ...
        console.warn(
          '[Atlantis Auth] profile recovery:',
          error
        );
      }
    }
  ...
  async function saveProfile(
    ...
    try{
      await window.atlantisSupabase.auth.updateUser({
        data:{username:cleanUsername}
      });
    }catch(error){
      console.debug(
        '[Atlantis Auth] metadata update:',
        error
      );
    }
  }
  ...
  async function updateNav(
    ...
    pop.dataset.atlantisProfileViewBtnBound =
      '1';
    ...
    pop.querySelector(
      ...
      async () => {
        ...
        if(error){
          ...
          window.alert(
            'Yeni e-posta adresine doğrulama gönderildi.'
          );
        }
      }
    );
  ...
  function showAtlantisProfileCard(
    profile,
    user,
    canEdit=false
  ){
    ...
    overlay.innerHTML = `
      ...
      <section
        class="auth-dialog atlantis-rich-profile"
        role="dialog"
        aria-modal="true"
      >
        <button
          type="button"
          class="auth-close"
          id="atlantis-profile-card-close"
          aria-label="Kapat"
        >×</button>

        <div class="atlantis-profile-head">
          <div class="atlantis-profile-big-avatar">
            ${
              avatar
                ? `
                  <img
          ...
          <div>
            ...
            <div class="atlantis-profile-status">
              <i></i>
              ${lastSeen ? esc(
        ...
        <div class="atlantis-profile-info-grid">
          <div>
            <span>Üyelik</span>
            <strong>
              ${
          ...
          canEdit
            ? `
              <button
                type="button"
                class="primary-button wide"
                id="atlantis-open-profile-editor"
              >
                Profili Düzenle
              </button>
            `
  ...
  function openAtlantisProfileEditor(){
    const user =
      window.atlantisAuthSession?.user;
    ...
    overlay.innerHTML = `
      ...
      <section class="auth-dialog">
        <button
          class="auth-close"
          type="button"
          id="atlantis-profile-editor-close"
        >×</button>
        ...
        <div
          id="atlantis-avatar-preview"
          class="atlantis-profile-big-avatar"
          style="margin:12px auto 16px;"
        ></div>

        <label class="ghost-button atlantis-file-button">
          📷 Profil fotoğrafı seç

          <input
            id="atlantis-avatar-file"
            type="file"
    ...
    overlay.querySelector(
      ...
      event => {
        ...
        if(file.size > 1024 * 1024){
          const message =
            overlay.querySelector(
              '#atlantis-profile-message'
            );

          if(message){
            message.textContent =
              'Profil fotoğrafı en fazla 1 MB olmalı.';
