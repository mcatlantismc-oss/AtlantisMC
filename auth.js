...
(() => {
  ...
  function ensureNavButton() {
    ...
    // The account control belongs beside the hamburger, never inside the nav menu.
    ...
    // Remove any old duplicate auth control from the nav.
    document.querySelectorAll('.site-nav #auth-nav-button').forEach(old => {
    ...
    if (btn.dataset.atlantisFinalBound !== '1') {
      ...
      btn.onclick = event => {
        ...
        header.querySelector('.menu-toggle')?.classList.remove('is-open');
  ...
  function updateNav() {
    ...
    const role = String(profile?.site_role || 'user');
    const roleLabel = role === 'admin' ? 'Kurucu' : role === 'moderator' ? 'Moderatör' : 'Oyuncu';
    btn.innerHTML = `${avatar ? `<img class="auth-nav-avatar" src="${esc(avatar)}" alt="">` : `<span class="auth-nav-fallback">${esc(username.charAt(0).toLocaleUpperCase('tr-TR') || 'A')}</span>`}` +
      `<span class="auth-nav-user"><b>${esc(username)}</b><small class="role-name-${esc(role)}">${esc(roleLabel)}</small></span>`;
    btn.classList.toggle('auth-logged-in', true);
    btn.dataset.role = role;
    btn.setAttribute('aria-label', `${username} hesap bilgileri`);
  }
  ...
  function bindHamburger() {
    document.querySelectorAll('.menu-toggle').forEach(original => {
      ...
      toggle.addEventListener('click', event => {
        ...
        // Keep the toggle and menu state synchronized on every click.
        if (!open) {
          document.querySelectorAll('.site-nav a').forEach(link => link.classList.remove('menu-item-visible'));
        }
      });

      // One global outside-click handler: no page-specific menu behavior.
      if (!document.body.dataset.atlantisMenuOutsideBound) {
        document.body.dataset.atlantisMenuOutsideBound = '1';
        document.addEventListener('pointerdown', event => {
          if (!document.body.classList.contains('menu-open')) return;
          if (event.target.closest('.site-nav, .menu-toggle, .auth-nav-button')) return;
          document.body.classList.remove('menu-open');
          document.querySelectorAll('.site-header').forEach(h => h.classList.remove('menu-open'));
