...
(() => {
  ...
  function openProfileDrawer(p, own = true) {
    ...
    const online = !!lastSeen && !Number.isNaN(new Date(lastSeen).getTime()) && (Date.now() - new Date(lastSeen).getTime() < 2 * 60 * 1000);
    const roleName = getRoleName(role);

    const drawer = document.createElement('aside');
    drawer.id = 'atlantis-account-drawer';
    drawer.className = 'account-drawer motion-enter';
    drawer.innerHTML = `
      ...
      <section class="account-drawer-panel" role="dialog" aria-modal="true" aria-label="${esc(name)} profil">
        ...
        <div class="account-drawer-body">
          <div class="account-status-card">
            <span>Son aktif</span>
            <strong class="${online ? 'status-online' : ''}">${online ? 'Çevrimiçi' : esc(formatLastSeen(lastSeen))}</strong>
          </div>
  ...
  async function init() {
    ...
    if (sessionUser) client.rpc('touch_my_last_seen').catch(() => {});
    updateNav();
