...
(() => {
  ...
  function renderLoading() {
    const box = document.getElementById('chat-messages');
    if (!box) return;
    box.innerHTML = '<div class="chat-empty chat-loading"><span class="loading-pulse"></span><span>Mesajlar yükleniyor...</span></div>';
  }
  ...
  async function syncMessages({initial=false}={}) {
    ...
    if (initial && !initialLoaded) renderLoading();
    ...
    chatChannel
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},async payload => {
        ...
        if (payload.new?.user_id !== currentUser?.id) {
          ...
          const mentioned = !!(myName && new RegExp(`(^|\\s)@${escapedName}(?=\\s|$|[.!?,])`, 'i').test(bodyText));
          const incoming = document.getElementById(`chat-message-${payload.new?.id}`);
          if (mentioned) {
            incoming?.classList.add('is-mentioned');
            if (localStorage.getItem('atlantis-chat-sound') === '1') playSound(false,true);
          }
          if (document.hidden) notify(payload.new,mentioned);
        }
      })
  ...
  function setupNotificationControls() {
    ...
    tools.innerHTML = `<button id="chat-notify" class="ghost-button" type="button">Bildirimleri Aç</button><button id="chat-sound" class="ghost-button" type="button">Ses Aç</button>${['admin','moderator'].includes(String(currentProfile?.site_role||'')) ? '<button id="chat-lock" class="ghost-button" type="button">Sohbet Kilidi</button>' : ''}`;
    ...
    document.getElementById('chat-notify').onclick = async () => {
      ...
      if (permission === 'granted') localStorage.setItem('atlantis-chat-notifications','1');
  ...
  function playSound(gesture=false, mention=false) {
    try {
      ...
      const notes = mention ? [660,880,1047] : [784,988];
      notes.forEach((freq,i)=>{
        const osc=audioContext.createOscillator();
        const gain=audioContext.createGain();
        osc.type='sine'; osc.frequency.value=freq;
        const start=now+i*(mention?.075:.06);
        const end=start+(mention?.20:.16);
        gain.gain.setValueAtTime(.0001,start);
        gain.gain.exponentialRampToValueAtTime(mention?.11:.085,start+.012);
  ...
  function notify(message, mentioned=false) {
    if (localStorage.getItem('atlantis-chat-sound') === '1') playSound(false,mentioned);
    if (document.hidden && localStorage.getItem('atlantis-chat-notifications') === '1' && 'Notification' in window && Notification.permission === 'granted') {
      new Notification(`Atlantis MC — ${message.username || 'Anonim'}`,{body:String(message.message || ''),tag:`atlantis-${message.id}`});
    }
  }
